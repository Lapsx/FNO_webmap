...
function drawCanvas() {
    ...
    charges.forEach(c => {
        ...
        const py = canvas.height - (c.z / N) * canvas.height; // CORRIGIDO: Eixo Z invertido para bater com Matplotlib
...
// Clique no Canvas (Adicionar Carga)
canvas.addEventListener('click', (e) => {
    ...
    const grid_z = Math.floor(((canvas.height - py) / canvas.height) * N); // CORRIGIDO: Inversão vertical
...
// Botão Direito no Canvas (Remover Carga)
canvas.addEventListener('contextmenu', (e) => {
    ...
    const grid_x = Math.floor((px / canvas.width) * N);
    const grid_z = Math.floor(((canvas.height - py) / canvas.height) * N);

    // Encontra a carga mais próxima
    let closestIdx = -1;
    ...
    // Se estiver razoavelmente perto (ex: raio de 10 unidades de grid), remove
...
saveStateBtn.addEventListener('click', () => {
    if (currentPayload) {
        ...
        // Resetar interface para criar o Estado B sem sobrescrever o A
        charges = [];
        kuhnSlider.value = 1.0; kuhnValue.textContent = '1.0';
        debyeSlider.value = 1.0; debyeValue.textContent = '1.0';
        florySlider.value = 0.0; floryValue.textContent = '0.0';
        drawCanvas();
        requestPrediction();
    }
});
